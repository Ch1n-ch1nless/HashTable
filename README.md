...
3. [Сравнительный анализ хеш-функций](#сравнительный-анализ-хеш-функций)
    ...
    5. [Хеш-функция, которая возвращает:](#хеш-функция-которая-возвращает)
        1. [константу](#константу)
        2. [ASCII код первого символа](#ascii-код-первого-символа-в-слове)
        3. [длину слова](#длину-слова)
        4. [сумму ASCII кодов в слове](#сумму-ascii-кодов-в-слове)
        5. [RorHash](#RorHash)
        6. [RolHash](#RolHash)
        7. [CRC32Hash](#CRC32Hash)
...
За решением обратимся к книге Кормена "Введение в алгоритмы". В главе 11 можно найти следующую цитату: «Хорошие результаты обычно получаются, если выбрать в качестве m простое число, далеко стоящее от степеней двойки». m - число возможных хеш-значений, в моей реализацией m - это количество списков в хеш-таблице. Так же хотелось бы, чтобы *load factor > 7*. *load factor* - это среднее количество элементов, приходящихся на каждую ячейку таблицы. Зачем его делать таким большим, если его стараются уменьшать? Это делается для лучшей демонстративности. 
...
### ASCII код первого символа в слове.
...
Результаты хуже, чем при размере $193$. Значит, при увеличении размера распределение вышло таким. 

**Гипотеза**:

Распределение вышло таким из-за того, что английские слова имеют определенную зависимость последовательности символов в слове, то из-за этого какие-то суммы кодов встречаются часто, а другие нет, из-за этого на графике видны подобные возвышенности.
...
Среднее время работы: $(57,9 \pm 0,8) * 10^9$ тактов. В среднем программа работает около 18 секунд.
...
|                           | Среднее время работы в тактах | Ускорение относительно v.0| Ускорение относительно предыдущей реализации  |
|:-------------------------:|:-----------------------------:|:-------------------------:|:---------------------------------------------:|
| Наивная реализация(v.0)   |   $(86,4 \pm 1,5) * 10^9$     |   1                       |   1                                           |
| Реализация с O3   (v.1)   |   $(57,9 \pm 0,8) * 10^9$     |   1,49                    |   1,49                                       |  
| Ускорение strcmp  (v.2)   |   $(43,8 \pm 0,7) * 10^9$     |   1,97                    |   1,32                                        |
...
| Реализация с O3           (v.1)   |   $(57,9 \pm 0,8) * 10^9$     |   1,49                     |   1,49                                       |  
| Ускорение strcmp          (v.2)   |   $(43,8 \pm 0,7) * 10^9$     |   1,97                     |   1,32                                        |
| Ускорение strcmp и CRC32 (v.3.1)  |   $(40,5 \pm 0,8) * 10^9$     |   2,13                     |   1,08                                        |

Как видно, оптимизация с помощью ассемблерной вставки дала прирост лишь в 8%, что очень мало. Я решил попробовать оптимизирова данную хеш-функцию дополнительно с помощью написания функции полностью на ассемблере. Время работы $\approx 12,5$ с.
...
| Реализация с O3           (v.1)   |   $(57,9 \pm 0,8) * 10^9$     |   1,49                     |   1,49                                       |  
| Ускорение strcmp          (v.2)   |   $(43,8 \pm 0,7) * 10^9$     |   1,97                     |   1,32                                        |
| Ускорение strcmp и CRC32 (v.3.2)  |   $(39,5 \pm 0,8) * 10^9$     |   2,19                     |   1,11                                        |

Действительно, программа, написанная на ассемблере отдельно, работает быстрее, чем написанная с помощью ассемблерной вставки. Попробуем применить дополнительно оптимизацию к другой функции с помощью переписывания на ассемблер. Среднее время $\approx 12$ c.
...
| Ускорение strcmp           (v.2)  |   $(43,8 \pm 0,7) * 10^9$     |   1,97                     |   1,32                                        |
| Ускорение strcmp и CRC32  (v.3.1) |   $(40,5 \pm 0,8) * 10^9$     |   2,13                     |   1,08                                        |
| Ускорение HashTableSearch  (v.4)  |   $(33,5 \pm 0,3) * 10^9$     |   2.58                     |   1.18                                        |

Среднее время работы $\approx 10,5$ секунд.
Переписывание на ассемблерный файл ускорило время работы программы на 18%, это очень хороший результат, но теперь рассчитаем, стоили ли эти усилия.

## Расчет Коэффициента Полезного Действия

**КПД** - это такой коэффициент, равный $K = \frac{a_i}{N} \cdot 1000$, где $N$ - количество написанных строк на ассемблере, $a_i$ - ускорение программы версии i относительно программы версии i-1. Данный коэффициент показывает соотношение между улучшением производительности и ухудшением читаемости кода.
...
Как видно из таблицы, КПД последних 2-ух операций < 100, это говорит о том, что данные ассемблерные оптимизации не эффективны. Они ухудшили значительно читаемость кода. Код перестал быть переносимым и выигрыш по времени не значителен. Значит, стоит остановиться, а лучше откатиться до 1 или 2 версии программы.
...
Хочу отдельно поблагодарить [Дединского Илью Рудольфовича](https://github.com/ded32)(мой преподаватель) и [Мухорина Матвея](https://github.com/matmuher)(мой ментор). 

# Литература

1. **Computer Systems: A Programmer's Perspective** 3rd Edition by **Randal Bryant**, **David O'Hallaron**
2. **Introduction to algorithms** third edition by **Thomas H. Cormen**, **Charles E. Leiserson**, **Ronald L. Rivest**, **Clifford Stein**
...
3. **Лекции Северова Д.С.**  - [ссылка](https://www.youtube.com/watch?v=BjcJ1fK139s&list=PLthfp5exSWEoOGJTuRZAPrZNUmTrTaUZ4)
